...
# 페이지 설정 & 전역 CSS
# ════════════════════════════════════════════════════════════════
st.set_page_config(page_title="HF Python 기초 교육", page_icon="🐍",
...
<div class="hdr-wrap">
  <div class="hdr-text">
    ...
    <p class="hdr-sub">라이브러리 소개 · Trino DB 연결 · 쿼리 작성법 · CSV / Excel 파일 저장</p>
  </div>
...
**Python**으로 가공하여 **CSV / Excel 파일**로 저장하는 기본 방법을 다룹니다.
복잡한 Python 문법보다 **실무에서 바로 쓰는 핵심 패턴**에 집중합니다.
    """)
...
""", unsafe_allow_html=True)
    ...
    st.markdown("아래가 실무에서 사용하는 **표준 코드 전체**입니다. 각 부분의 의미를 이 교육에서 차례로 설명합니다.")
...
# ════════════════════════════════════════════════════════════════
    ...
    st.markdown("""
라이브러리란 **다른 사람이 미리 만들어 놓은 유용한 코드 묶음**입니다.
...
""", language="python")
    ...
    st.markdown('''<div class="wrn">⚠️ <b>주의 — 오타</b><br>
...
일반 사내 DB(Oracle, MSSQL 등)가 한 대의 서버에서 처리하는 방식이라면,
...
<span class="step-num">🌐</span> <b>호스트주소</b> — Trino 서버 주소 (IT팀 확인)<br><br>
<span class="step-num">🔢</span> <b>포트번호</b> — 보통 8080 또는 8443<br><br>
<span class="step-num">📂</span> <b>카탈로그</b> — 최상위 데이터 구분 (예: iceberg)<br><br>
<span class="step-num">📁</span> <b>스키마</b> — 카탈로그 안의 레이어 (예: bronze, silver, gold)
</div>
""", unsafe_allow_html=True)
    with c2:
        st.markdown(f"""
...
# ════════════════════════════════════════════════════════════════
    ...
    t1, t2, t3 = st.tabs(["SQL 기본 개념", "Python에서 쿼리 호출", "JSON 컬럼 처리"])

    # ── 탭 1: SQL 기본 개념 ──
    with t1:
        st.markdown("#### SQL이란?")
        st.markdown("""
데이터베이스에서 데이터를 꺼내오기 위한 **질의 언어**입니다.
Python 코드 안에 SQL 문장을 문자열로 작성하여 Trino에 전달합니다.
        """)

        st.markdown("#### 기본 SELECT 문 구조")
        st.markdown(f"""
<div class="syn">
SELECT &nbsp; 컬럼1, 컬럼2, ...&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ← <b>가져올 컬럼</b> (모두 가져오려면 *)<br>
FROM &nbsp;&nbsp;&nbsp; 테이블명 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ← <b>어떤 테이블에서</b><br>
WHERE &nbsp; 조건 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ← <b>어떤 조건으로</b> (생략 가능)<br>
ORDER BY 컬럼 DESC &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ← <b>정렬</b> (생략 가능)<br>
LIMIT &nbsp;&nbsp; 100 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ← <b>최대 조회 건수</b> (권장)
</div>
""", unsafe_allow_html=True)

        st.markdown("#### 자주 쓰는 WHERE 조건")
        st.code("""-- 특정 값 일치
WHERE basis_ym = '202306'

-- 여러 값 중 하나
WHERE region_cd IN ('11', '41', '28')

-- 범위 조건
WHERE grnt_amt >= 100000
...
Trino는 수억 건 이상의 데이터를 다룹니다. <b>반드시 <code>WHERE</code> 조건과 <code>LIMIT</code>를 걸고</b> 조회하세요.<br>
조건 없는 쿼리는 수억 건을 다운로드하려다 오류가 발생하거나 서버에 부하를 줍니다.
</div>''', unsafe_allow_html=True)
...
""", language="python")
        ...
        st.markdown('''<div class="wrn">⚠️ <b>자주 하는 실수</b><br>
